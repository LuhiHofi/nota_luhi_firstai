...
function Run(self, units, parameter)
    ...
    if not self.is_initialized then
        ...
        for i, unitID in ipairs(peepers) do
            ...
            Spring.GiveOrderToUnit(unitID, CMD.MOVE, Vec3(x, Spring.GetGroundHeight(x, 50), 50):AsSpringVector() , {"shift"}) -- otherwise the command oftentimes stays RUNNING
        end
    ...
    for i, unitID in ipairs(peepers) do
        local unitunitState = self.unitState[unitID]
        if unitunitState == 0 then
            isSuccess = false
            break
        end
    end
